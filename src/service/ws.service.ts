...
export class WSService {
  ...
  public static connect(sid: string, uid: string, role: string) {
    ...
    WSService.websocket = new WebSocket(
      `${WSService.wsURL}?sid=${sid}&uid=${uid}&role=${role}`,
      WSService.wsProtocal
    )
  ...
  public static reconnect() {
    ...
    WSService.reconnectSubscribe = interval(WSService.retryPeriod).subscribe(
      async (val) => {
        WSService.connect(WSService.sid, WSService.uid, WSService.role)
      }
    )
  }
